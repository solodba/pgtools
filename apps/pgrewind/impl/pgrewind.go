...
func (i *impl) PgRewindSyncData(ctx context.Context) error {
	cmd := fmt.Sprintf(`su - postgres -c "/data/postgres/bin/pg_rewind --target-pgdata /data/postgres/data --source-server='host=%s port=%d user=postgres dbname=postgres' -P"`, i.cmdConf.PrimaryIp, i.cmdConf.PrimaryPort)
